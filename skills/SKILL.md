...
- **The piece is Glossa's, and Glossa speaks.** Not a review of someone else's article: an article. State what happened — "a quiet search for asylum ran through three capitals", never "the author describes a quiet search for asylum". Handing every sentence back to the source is not caution, it is abdication; it turns a magazine piece into a term paper and it bores. What carries the caution is the **mark on the claim** (`<span class="doc">` a document supports it, `<span class="attr">` one source only, `<span class="said">` asserted and unsupported); unmarked prose means checked. Name a source in the prose only when naming informs: the claim is contested, who said it IS the story, or two sources disagree.
  Banned outright, because they do the reader's thinking out loud instead of doing the work: telling the reader how to read the source ("take this as one journalist's account"); describing the TEXT instead of the world ("the column treats this as settled", "it carries no byline"); and a context box introducing **who the source is**. Context boxes explain the SUBJECT — an institution, a treaty, a term — never the byline. Provenance lives in the marks, in the source line under the headline, and in the sources at the foot; the page prints all three by itself.
...
- **`billion` is a false friend.** English "billion" = Spanish "mil millones" (10⁹). Spanish "billón" = English "trillion" (10¹²). $5 trillion → "5 billones." $160 billion → "160 mil millones." Get this wrong and the article is misinformation. And in the Spanish edition the English figure goes **in parentheses the first time**: "5 billones de dólares (5 trillion)", "130 mil millones de dólares (130 billion)" — the reader who heard the number in English has to recognise it. Only the first time, and only when the scale word changes: "900 millones" needs nothing.
- **Beijing → Pekín** in Spanish convention.
- **Operation names** stay in English, italicized: "*Operation Epic Fury*" — don't translate.
- **Mexican register** (D-020), not Iberian: «computadora», «celular», no «vosotros», the sober register of a Mexican daily. (This line said "Iberian" until 2026-08-31; that was the pre-D-020 convention, corrected in `spanish-translation.md` on 2026-08-25 and here late — the "one rule in one place" failure, again.)
- **No calques of image or idiom** — the canonical failure is «move» → «movimiento» («el primer movimiento del argumento» is unreadable Spanish; write «el paso», «la jugada», «el punto de partida»). The full list lives in the REGLAS_ES block of `spanish-translation.md`, generated from `src/lib/espanol.js` — the same list the validator enforces.
...
- **Number formatting is MEXICAN** (decided 2026-08-31): `,` for thousands, `.` for decimals — "1,234.56", "51.7%", "13 mil millones de dólares". Never "51,7 por ciento" or "160.000".
