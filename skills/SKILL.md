...
- **Pedagogical hinges on organizing concepts.** Every piece has two or three passages where one distinction or concept organizes everything that follows. On those passages, expand: concrete anchor first, name after, sub-terms glossed inline, abstract opposition recast as two concrete questions, close with a reflection prompt. Don't apply this to every paragraph — that converts the piece into a textbook. See `§ Pedagogical hinges`. If Arturo asks for a piece that is "más accesible" or "para seminario," increase hinges; if he asks for strict academic-magazine register, reduce to zero.
